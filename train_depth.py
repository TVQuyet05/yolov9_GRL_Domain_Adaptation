...
import val_depth as validate  # for end-of-epoch mAP with depth
from models.experimental import attempt_load
from models.yolo import DetectionModelDepth
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Depth paths
    train_depth_path = data_dict.get('train_depth', '')
    val_depth_path = data_dict.get('val_depth', '')
    assert train_depth_path, 'train_depth path must be specified in data yaml'
    assert val_depth_path, 'val_depth path must be specified in data yaml'
    ...
    if pretrained:
        ...
        model = DetectionModelDepth(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)
        ...
        model = DetectionModelDepth(cfg, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
    amp = check_amp(model)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, targets, paths, _, depths) in pbar:  # batch ---------------------------------------------------
            callbacks.run('on_train_batch_start')
            ni = i + nb * epoch  # number integrated batches (since train start)
            imgs = imgs.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
            depths = depths.to(device, non_blocking=True).float()  # depth maps to device
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    # Also resize depth maps
                    depths = nn.functional.interpolate(depths, size=ns, mode='bilinear', align_corners=False)

            # Forward — pass depth_map to model
            with torch.cuda.amp.autocast(amp):
                pred = model(imgs, depth_map=depths)  # forward with depth
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data_depth.yaml', help='dataset.yaml path')
...
def run(**kwargs):
    # Usage: import train_depth; train_depth.run(data='data_depth.yaml', imgsz=320, weights='yolo.pt')
    opt = parse_opt(True)
