...
def run(
    ...
    if training:  # called by train_depth.py
    ...
    # Depth path
    val_depth_path = data.get('val_depth', '')
    ...
    for batch_i, batch_data in enumerate(pbar):
        # Handle both depth and non-depth dataloaders
        if len(batch_data) == 5:
            im, targets, paths, shapes, depths = batch_data
        else:
            im, targets, paths, shapes = batch_data
            depths = None
        ...
        with dt[0]:
            ...
            if depths is not None:
                depths = depths.half() if half else depths.float()
