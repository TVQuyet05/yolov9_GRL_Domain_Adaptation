...
from utils.grl import Discriminator
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    train_loader, dataset = create_dataloader(train_path,
                                              ...
                                              quad=opt.quad,
                                              prefix=colorstr('train source: '),
                                              shuffle=True,
                                              min_items=opt.min_items)

    # Trainloader for Target
    target_path = data_dict.get('target_train') or opt.target_data
    if target_path:
        # Nếu path truyền vào là file yaml (ví dụ d:\yolov9\data1.yaml)
        if isinstance(target_path, str) and target_path.endswith('.yaml'):
            target_dict = check_dataset(target_path)
            target_path = target_dict['train']
            
        target_loader, target_dataset = create_dataloader(target_path,
                                                          imgsz,
                                                          batch_size // WORLD_SIZE,
                                                          gs,
                                                          single_cls,
    ...
    class FeatureExtractor:
        def __init__(self, model, layer):
            self.feature = None
            # layer is an index in the Sequential model
    ...
    # Chs for yolov9s p4 is 192 (or 256 depending on architecture, let discriminator handle this via input feature shape)
    # We will delay discriminator instantiation or assume a specific channel size
    # Typically gelan-s P4 has 192 channels, let's allow it to be configured by the first batch, or assume 256?
    # Discriminator(chs=[192] or [256])
    discriminator = Discriminator(chs=[192], amp=amp).to(device)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Cập nhật alpha cho GRL
            p = float(ni) / (epochs * nb)
            alpha = 2. / (1. + math.exp(-10 * p)) - 1
            discriminator.grl.alpha = alpha

            imgs = imgs.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
            
            # Lấy Target batch (vòng lặp iterator vô hạn)
            try:
                imgs_target, _, _, _ = next(target_iter)
            except StopIteration:
                target_iter = iter(target_loader)
                imgs_target, _, _, _ = next(target_iter)
            imgs_target = imgs_target.to(device, non_blocking=True).float() / 255
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                feature_s = extractor.feature
                
                # Forward Target
                _ = model(imgs_target)
                feature_t = extractor.feature

                # Discriminator output
                d_out_s = discriminator([feature_s])
                d_out_t = discriminator([feature_t])
                
                label_s = torch.ones_like(d_out_s, device=device)
            ...
            if ni - last_opt_step >= accumulate:
                ...
                scaler.step(discriminator.optim)
                scaler.update()
                optimizer.zero_grad()
...
def parse_opt(known=False):
    ...
    parser.add_argument('--target-data', type=str, default='', help='target dataset.yaml path for GRL (foggy)')
    parser.add_argument('--grl-layer', type=int, default=6, help='Layer index to extract feature (e.g. 6 for P4 in yolov9s)')
